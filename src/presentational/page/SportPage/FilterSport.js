...
const stateToPropsFilterSport = (state) => ({
  from: 'filter',
  searchText: state.sportunitySport.searchText,
  language: state.sportunityLocale.language,
});

const ReduxContainerFilterSport = connect(
  stateToPropsFilterSport,
  dispatchToProps
)(FilterSport);

const FilterSportTemp = createRefetchContainer(ReduxContainerFilterSport, {
  viewer: graphql`
  ...
  `,
    ...
    graphql`
        query FilterSportRefetchQuery ($querySports: Boolean!, $count: Int, $sportFilter: SportFilter) {
            viewer {
                ...FilterSport_viewer @arguments(querySports: $querySports, count: $count, sportFilter: $sportFilter)
            }
        }    
    `
);
    ...
    render() {
      return (
        <QueryRenderer
          ...
          render={({error, props}) => {
            if (props) {
              return <FilterSportTemp query={props} viewer={props.viewer} {...this.props}/>;
            } else {
              return (
